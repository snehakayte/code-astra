...
function inboundFetchDefaultDataByDate(startDate, endDate, page) {
	checkSession()
		.done(function(response) {
			...
			showLoadingAlert();
			const url = `${contextName}/sor/inbound/api/monitoring/default?startDate=${startDate}&endDate=${endDate}&pageNumber=${page}`;
			$.ajax({
				url: url,
				type: "POST",
				dataType: "json",
...
function inboundFetchDataByDateApiKey(startDate, endDate, txnTypes, page) {
	checkSession()
		.done(function(response) {
			showLoadingAlert(); // Show loading alert
			const url = `${contextName}/sor/inbound/api/monitoring/allentries/apikey?startDate=${startDate}&endDate=${endDate}&txnTypes=${txnTypes}&pageNumber=${page}`;
			...
			$.ajax({
				...
				success: function(data) {
					...
					inboundDataByDateApiKeyupdatePageNumbers(startDate, endDate, txnTypes, data.totalPages);
...
function inboundDataByDateApiKeyupdatePageNumbers(startDate, endDate, txnTypes, totalPages) {
    ...
    function renderPageGroup(group) {
        ...
        for (var i = startPage; i <= endPage; i++) {
            ...
            pageNumberLink.click(function () {
                currentPage = parseInt($(this).text()); // Capture the page number from the link
                inboundFetchDataByDateApiKey(startDate, endDate, txnTypes, currentPage);
            });
            if (i === currentPage) {
                pageNumberLink.addClass('page-number'); // Add class to highlight current page number
            }
...
function inboundFetchDataByDateApiKeyStatus(startDate, endDate, txnTypes, status, page) {
	checkSession()
		.done(function(response) {
			showLoadingAlert(); // Show loading alert
			const url = `${contextName}/sor/inbound/api/monitoring/allentries/apikeystatus?startDate=${startDate}&endDate=${endDate}&txnTypes=${txnTypes}&status=${status}&pageNumber=${page}`;
			...
			$.ajax({
				...
				success: function(data) {
					...
					$('.success-count').text(data.successCount);
					$('.failed-count').text(data.failedCount);
					inboundupdateDataByDateApiKeyStatusPageNumbers(startDate, endDate, txnTypes, status, data.totalPages);
...
function inboundupdateDataByDateApiKeyStatusPageNumbers(startDate, endDate, txnTypes, status, totalPages) {
    ...
    function renderPageGroup(group) {
        ...
        for (var i = startPage; i <= endPage; i++) {
            ...
            pageNumberLink.click(function () {
                currentPage = parseInt($(this).text()); // Capture the page number from the link
                inboundFetchDataByDateApiKeyStatus(startDate, endDate, txnTypes, status, currentPage);
            });
            if (i === currentPage) {
                pageNumberLink.addClass('page-number'); // Add class to highlight current page number
            }
...
function inboundFetchDataByTxnRefNo(txnRefNo) {
	checkSession()
		.done(function(response) {
			showLoadingAlert(); // Show loading alert
			const url = `${contextName}/sor/inbound/api/monitoring/entries/txnrefno?txnRefNo=${txnRefNo}`;
